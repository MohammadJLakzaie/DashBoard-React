...
export default function Signin() {
  ...
  const signInValidation = () => {
    if (
      // if true , navigate to dashboard and set user to loggedIn
    ...
    <div className="w-full max-w-md bg-white/75 rounded-2xl lg:mx-60 shadow-card overflow-hidden transition-all duration-300 hover:shadow-2xl">
      <div className="px-4 py-6 sm:px-8 sm:py-10">
        ...
        <form id="signinForm" className="space-y-6" method="POST">
          <Inputs
            type="text"
            placeholder="نام کاربری خود راوارد کنید "
            label="نام کاربری "
            value={email}
          ...
          <Buttons
            ...
            name="ورود"
            width="w-full"
            color="text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500"
            type="button"
          />
          <Buttons
            onClick={() => navigate("/signup")}
            name="اکانت ندارید؟ ثبت نام کنید"
            width="w-full"
            color="text-black bg-indigo-100 hover:ring-sky-300 focus:ring-sky-300 "
          />
        </form>
        <br />
        {!status && (
